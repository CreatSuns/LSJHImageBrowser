...
@property (nonatomic, strong) WYAZoomingScrollView * currentZoomingScrollView;
@end
...
- (void)updateFrames
{
    ...
    self.saveButton.frame              = CGRectMake(self.cmam_width - 80, self.cmam_height - 70, 50, 25);
...
- (void)lookupOriginalImage{

    if ([self highQualityImageURLForIndex:self.currentImageIndex]) { // 如果提供了高清大图数据源,就去加载
        [self.currentZoomingScrollView setShowHighQualityImageWithURL:[self highQualityImageURLForIndex:self.currentImageIndex] placeholderImage:[self placeholderImageForIndex:self.currentImageIndex]];
    } else if ([self assetForIndex:self.currentImageIndex]) {
        ALAsset * asset     = [self assetForIndex:self.currentImageIndex];
        CGImageRef imageRef = asset.defaultRepresentation.fullScreenImage;
        CGImageRelease(imageRef);
        [self.currentZoomingScrollView setShowImage:[UIImage imageWithCGImage:imageRef]];
    }
    self.lookupButton.hidden = YES;
}
...
- (UIButton *)lookupButton{
    if (!_lookupButton) {
        _lookupButton = [UIButton buttonWithType:UIButtonTypeCustom];
